...
from ..models import Company, Sales, Manager, SalesHead
...
def resolve_historical_analysis_scope(*, user) -> HistoricalAnalysisScope:
    ...
    - Client group + user.role == "Sales" => sales (only own data, company fixed)
    """
    ...
    if _in_group(user, "Sales") :
        cu = Sales.objects.filter(user=user).select_related("company").first()
        return HistoricalAnalysisScope(role="sales", company=cu.company if cu else None)
    
    if _in_group(user, "SalesHead"):
        cu = SalesHead.objects.filter(user=user).select_related("company").first()
        return HistoricalAnalysisScope(role="sales", company=cu.company if cu else None)
